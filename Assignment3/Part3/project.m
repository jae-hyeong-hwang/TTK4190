...
Ja = 0;                 %Va/(n*D)advance number
PD  = 1.5;              %pitch/diameter ratio (typically 0.5-2.5)
AEAO = 0.65;            %blade area ratio (ratio of expanded blade area to propeller disk area)
z = 4;                  %number of propeller blades

[KT, KQ]  = wageningen(Ja,PD,AEAO,z)      % propeller coefficient (-)
...
for i=1:Ns+1
    ...
    Td = U_d*Xu/(t_thr-1);                      % Desired thurst
    n_c = sqrt(abs(Td/(rho*Dia^4*KT)))*sign(Td);% propeller speed (rps)
    Qd = rho * Dia^5 * KQ * abs(n_c) * n_c;     % Desiered tourqe
    Y = 1/Km*Qd;                                % Motor input
    Qm = Qm + h*(1/Tm*(Km*Y-Qm));               % Motor torque
    Q = rho * Dia^5 * KQ * abs(n) * n;          % Propeller Torque
    Im = 100000; Tm = 10; Km = 0.6;         % propulsion parameters
    n_dot = (1/Im) * (Qm - Q);             % RPM dynamics
